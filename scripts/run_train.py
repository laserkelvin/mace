...
def determine_arch() -> dict[str, str]:
    """
    Utility function to determine what architecture to execute with.

    At a high level, we run through the gambit of determining if the
    required packages for Intel XPU usage are installed and if there
    are detectable devices. Ultimately, fall back to CPU if neither
    are available.

    Returns
    -------
    dict[str, str]
        Dictionary containing `target_arch` and `comm_backend` keys.

    Raises
    ------
    ImportError
        If IPEX is installed, but was unable to be imported. This is
        typically due to missing dynamic libraries, such as `libze_loader.so`
        or something because of pathing.
    """
    ...
    if torch.cuda.is_available() and torch.cuda.device_count() != 0:
        ...
        has_ccl = find_spec("oneccl_bindings_for_pytorch")
        if has_ipex:
            try:
        ...
        if has_ccl:
            try:
                ...
                # warning is raised because oneCCL is not technically needed if
                # we are not using DDP
                logger.warning(f"oneCCL is installed, but could not be imported: {e}") 
        # fallback scenario: no XPU devices available but the Intel stack is installed
        if torch.xpu.device_count == 0:
            logger.warning("IPEX is installed but no XPU devices detected; defaulting to CPU.")
            target_arch = "cpu"
        target_arch = "xpu"
        # for Intel, backend will always be oneCCL
        comm_backend = "ccl"
    return {"target_arch": target_arch, "comm_backend": comm_backend}
